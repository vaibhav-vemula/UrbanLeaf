...
var runVerifyAndVote = (sendRequester, blockchainServiceUrl, payload) => {
  const { proposalId, vote, voter, idkitResult, rp_id } = payload;
  const verifyRes = sendRequester.sendRequest({
    method: "POST",
  ...
  const verifyData = json(verifyRes);
  ...
  const voteRes = sendRequester.sendRequest({
    method: "POST",
...
var onHttpTrigger = (runtime2, httpPayload) => {
  const payload = decodeJson(httpPayload.input);
  runtime2.log(`[UrbanLeaf CRE] World ID verify-vote → proposal #${payload.proposalId} | voter: ${payload.voter} | vote: ${payload.vote ? "YES" : "NO"} | rp_id: ${payload.rp_id}`);
  const httpClient = new cre.capabilities.HTTPClient;
  const result = httpClient.sendRequest(runtime2, (sendRequester) => runVerifyAndVote(sendRequester, runtime2.config.blockchainServiceUrl, payload), consensusIdenticalAggregation())().result();
  runtime2.log(`[UrbanLeaf CRE] World ID verified on CRE → vote cast on Arbitrum Sepolia: txHash=${result.txHash} nullifier=${result.nullifier.slice(0, 18)}...`);
