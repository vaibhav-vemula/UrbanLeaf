/**
 * UrbanLeaf AI — CRE Workflow: Environmental Impact Scorer
 *
 * Trigger: HTTP POST /score-proposal
 * Called by the UrbanLeaf backend after a proposal is created on-chain.
 *
 * Pipeline:
...
const onHttpTrigger = (runtime: Runtime<Config>, payload: HTTPPayload): string => {
  const { proposalId, parkId } = decodeJson(payload.input) as {
    proposalId: string
    parkId: string
  }

  runtime.log(`[UrbanLeaf CRE] HTTP trigger → scoring proposal #${proposalId} (park: ${parkId})`)
