...
var runScoring = (sendRequester, urbanleafApiUrl, blockchainServiceUrl, geminiApiKey, proposalId, parkId) => {
  ...
  const writeRes = sendRequester.sendRequest({
    method: "POST",
...
var onHttpTrigger = (runtime2, payload) => {
  const { proposalId, parkId } = decodeJson(payload.input);
  ...
  runtime2.log(`[UrbanLeaf CRE] Score written on-chain: ${result.aiScore}/100 (${result.urgencyLevel}) — "${result.insight}"`);
